#!/usr/bin/env python3
"""Quick closed-loop with minimal CEM config for learned rollout.

Config: horizon=10, samples=32, iterations=2, speed=0.75
Each MPC step should take ~30 seconds.
"""
...
print(f'CEM: H={H} S={S} E={E} I={I} speed={MAX_SPEED}', flush=True)
...
os.makedirs(f'{base}/open_space/cem', exist_ok=True)
os.makedirs(f'{base}/classic_templates/cem', exist_ok=True)
...
for model_type in ['flat', 'object_centric', 'causality_aware']:
    ...
    planner = CEMLearnedPlanner(model=model, normalizer=norm, convention=conv,
        horizon=H, num_samples=S, num_elites=E, num_iterations=I, init_std=0.3, device='cpu')
