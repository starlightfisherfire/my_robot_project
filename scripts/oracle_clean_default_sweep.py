...
TRIALS_PER = 3
COST_MODES = ["clean_default"]
WORKERS = int(os.environ.get("SWEEP_WORKERS", "13"))
OUT_DIR = Path(os.environ.get("SWEEP_OUT_DIR", "runs/oracle_clean_default_sweep"))
...
def main():
    ...
    # Baseline: MPPI_s03_H100 + clean_default
    for family in FAMILIES:
        for trial in range(TRIALS_PER):
    ...
    print('DEBUG: about to create ProcessPoolExecutor', flush=True)
    t0 = time.time()
    results = []

    with ProcessPoolExecutor(max_workers=workers) as executor:
        print('DEBUG: executor created, submitting tasks', flush=True)
        futures = {executor.submit(run_one_trial, t): t for t in tasks}
        print('DEBUG: tasks submitted, waiting for results', flush=True)
