#!/usr/bin/env python3
"""
Warm-Start vs Original at s03 speed.
CEM & MPPI, 3 families, 3 seeds each = 36 trials.
"""
import subprocess, sys, os, json, time
from pathlib import Path
from concurrent.futures import ProcessPoolExecutor, as_completed

OUT_DIR = Path("runs/warmstart_s03")
OUT_DIR.mkdir(parents=True, exist_ok=True)

TRIALS = [
    ("CEM",     "CEM_s03_exec20",      "CEM_s03"),
    ("WS_CEM",  "WS_CEM_s03_exec20",   "WS_CEM_s03"),
    ("MPPI",    "MPPI_s03_exec20",     "MPPI_s03"),
    ("WS_MPPI", "WS_MPPI_s03_exec20",  "WS_MPPI_s03"),
]
...
def run_one(args):
    ...
    cmd = [py, "scripts/run_single_clean_trial.py",
...
def main():
    ...
    print(f"=== Warm-Start s03 Ablation ===")
    ...
    print(f"{'='*85}")
    print(f"{'Config':<12} {'Scen':<10} {'N':>2} {'Succ':>5} {'Dist':>8} {'Contact':>8} {'FC_step':>8} {'Time':>6}")
    print("-" * 85)
    for label in [t[2] for t in TRIALS]:
        for family in FAMILIES:
            ...
            print(f"{label:<12} {family:<10} {n:>2} {ok:>5} {d:>8.4f} {c:>8.3f} {sum(fc)/len(fc) if fc else -1:>8.1f} {t_:>5.0f}s")

    print(f"\n--- PAIRWISE ---")
    for ws, orig in [("WS_CEM_s03","CEM_s03"), ("WS_MPPI_s03","MPPI_s03")]:
        for family in FAMILIES:
            ...
            ws_sr = sum(1 for r in w if r.get("success"))/len(w)
            or_sr = sum(1 for r in o if r.get("success"))/len(o)
            print(f"  {ws} vs {orig} @ {family}: succ {ws_sr:.0%} vs {or_sr:.0%} | dist {wd:.4f} vs {od:.4f} | time {wt:.0f}s vs {ot:.0f}s ({ot/wt:.1f}x)")
