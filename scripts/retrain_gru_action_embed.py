#!/usr/bin/env python3
"""Train 3 models with GRU encoder + action_embed, 30 epochs, early stopping.

Same training protocol as retrain_lastframe.py but with standard GRU encoder.
Purpose: direct comparison of GRU vs lastframe encoder.

Output: runs/retrain_gru_action_embed_30ep/
"""
...
OUT_DIR = Path('runs/retrain_gru_action_embed_30ep')
EPOCHS = 30
...
def train_model(model_type):
    ...
    print(f'\n=== Training {model_type} ({n_params:,} params, GRU+action_embed+mass) ===')
    t0 = time.time()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
                'architecture': 'gru_action_embed_mass',
            }, model_dir / 'best.pt')
        else:
            patience_counter += 1
...
def main():
    ...
    print(f'Encoder: GRU (standard) + action_embed + mass/friction')
