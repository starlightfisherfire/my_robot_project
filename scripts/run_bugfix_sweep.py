...
def main():
    ...
    # Save config
    config = {'bugfixes': ['obstacle_token', 'max_speed_0.5', 'extract_state16_fixed'],
              'max_speed_mps': 0.5, 'timestamp': TIMESTAMP}
    with open(RUN_DIR / 'config.json', 'w') as f: json.dump(config, f, indent=2)

    # Trial matrix
    conv = PAPER1_CONVENTION; conv.max_speed_mps = 0.5  # FIXED
    ...
    print(f'Bug fixes: obstacle token + max_speed=0.5')
    ...
    summary = {
        ...
        'bugfixes': ['obstacle_token', 'max_speed_0.5'],
    }
