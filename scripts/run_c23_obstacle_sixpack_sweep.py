#!/usr/bin/env python3
"""
Config23 Obstacle Sixpack Sweep: 6 templates x 3 budgets = 18 evaluations.

Reads: data/sim/metadata/reset_templates_obstacle_sixpack_v0.json

Budget A (c23_strict600):
  horizon=80, execute_steps=20, max_mpc_steps=30 (600 total env steps)
  num_samples=1024, num_elites=96, num_iterations=5
  strict_pose_stop: pos<=1.5mm AND theta<=3.0deg

Budget B (c23_strict800):
  horizon=80, execute_steps=20, max_mpc_steps=40 (800 total env steps)
  num_samples=1024, num_elites=96, num_iterations=5
  strict_pose_stop: pos<=1.5mm AND theta<=3.0deg

Budget C (c23_strict1000):
  horizon=80, execute_steps=20, max_mpc_steps=50 (1000 total env steps)
  num_samples=1024, num_elites=96, num_iterations=5
  strict_pose_stop: pos<=1.5mm AND theta<=3.0deg

Usage:
  MUJOCO_GL=egl PYTHONPATH=. python scripts/run_c23_obstacle_sixpack_sweep.py \
...
def build_report(
    ...
    # Obstacle metadata from template
...
def summarize_by_budget(reports: list[dict]) -> list[dict]:
    """Aggregate stats grouped by budget_name."""
...
def summarize_by_layout(reports: list[dict]) -> list[dict]:
    """Aggregate stats grouped by budget_name x layout_family."""
...
def write_compact_summary(
    ...
    lines = []
    lines.append("=" * 72)
    lines.append("Config23 Obstacle Sixpack Sweep — Compact Summary")
    lines.append("=" * 72)
    lines.append("")
    ...
    # Per-template details per budget
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Config23 obstacle sixpack sweep: 6 templates x N budgets (default 3 = 18 evals)",
    )
    p.add_argument(
        "--templates",
        default="data/sim/metadata/reset_templates_obstacle_sixpack_v0.json",
        help="Path to sixpack template JSON.",
    )
    p.add_argument(
        "--output-root",
        default="runs/obstacle_sweeps",
        help="Root output directory.",
    )
    p.add_argument(
        "--run-name",
        default="c23_obstacle_sixpack",
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Render videos after evaluation (default: False).",
    )
    p.add_argument(
        "--render-all-videos",
        action="store_true",
        default=False,
        help="Render videos for all templates (default: only failures if --render-videos).",
    )
    p.add_argument(
        "--budgets",
        nargs="+",
        default=None,
        help="Budget names to run (default: all). E.g. --budgets c23_strict600",
    )
    p.add_argument(
        "--max-speed-mps",
        type=float,
...
def main() -> None:
    ...
    print(f"\n{'='*72}")
    print(f"Config23 Obstacle Sixpack Sweep")
    print(f"Run directory: {run_dir}")
    print(f"Budgets: {budget_names}")
    print(f"Templates: {len(templates)}")
    print(f"Total evaluations: {len(templates) * len(budget_names)}")
    print(f"{'='*72}\n")
    ...
    for budget_name in budget_names:
        ...
        print(f"  horizon={config['horizon']}  execute_steps={config['execute_steps']}  "
              f"max_mpc_steps={config['max_mpc_steps']}  "
              f"total={config['execute_steps']*config['max_mpc_steps']}")
        print(f"{'#'*72}")
        ...
        for i, template in enumerate(templates):
            ...
            print(f"\n{'─'*72}")
            print(f"[{eval_index}/{len(templates)*len(budget_names)}] "
                  f"Budget={budget_name}  Template={tid}  seed={seed}")
            print(f"{'─'*72}")

            log_path = run_dir / "logs" / f"{budget_name}__{tid}.log"
            try:
                result = run_one_eval(template, config, seed, log_path, max_speed_mps=args.max_speed_mps)
                report = build_report(result, template, config, budget_name, run_name)
                all_reports.append(report)

                report_path = run_dir / "reports" / f"{budget_name}__{tid}.json"
